      ...
      <div className="w-full max-w-3xl mx-auto px-5 py-6">
        <div className="text-left">
          <div className="mb-6 flex items-center space-x-4">
            <h2 className={`text-[#333] dark:text-white font-bold text-xl sm:text-2xl whitespace-nowrap tracking-tight noto-sans ${isEnglish ? 'eng-font' : ''}`}>
              {displayReference}
            </h2>
            <div className="h-[1px] w-full bg-stone-100 dark:bg-white/10"></div>
          </div>
          ...
          <div className="space-y-7">
            {versePairs.map((v, idx) => (
              ...
              >
                ...
                */}
                <span className="w-10 shrink-0 text-[13px] font-bold text-stone-400 dark:text-amber-gold/70 group-hover:text-blue-500 dark:group-hover:text-amber-gold transition-colors eng-font pr-4">
                  {v.num}
                </span>
            ...
            {versePairs.length === 0 && (
              <div className="py-10 text-center">
                <p className="text-stone-300 dark:text-white/40 italic serif-font">말씀을 읽어오는 중입니다...</p>
              </div>
            )}
          </div>
        </div>
